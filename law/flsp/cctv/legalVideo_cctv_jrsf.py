...
def get_lists():
    urls = []
    # 获取数据libid=04、02、03
    url = 'http://tv.cctv.com/lm/jrsf/videoset/index.shtml'
...
def do_insert(href):
    ...
    if title!=None and wenben!=[]:
        ...
        if len(title_info) ==3:
            ...
            sql ="insert into t_law_video_info (`video_id`,`video_name`,`video_introduction`,`video_url`,`recording_time`,`video_source`,`source_code`) values(%s,%s,%s,%s,%s,%s,%s)"
            cursor.execute(sql,(str(uuid.uuid1()),video_name,video_introduction,video_url,recording_time,video_source,source_code))
            db.commit()
            print("-----------------------------------------------")
        ...
        if len(title_info) ==3:
            video_name = title_info[2]
            recording_time = title_info[1]
            sql ="insert into t_law_video_info (`video_id`,`video_name`,`video_url`,`recording_time`,`video_source`,`source_code`) values(%s,%s,%s,%s,%s,%s)"
            cursor.execute(sql,(str(uuid.uuid1()),video_name,video_url,recording_time,video_source,source_code))
            db.commit()
            print("-----------------------------------------------")
        elif (title.get_text().find("]")!=-1 and title.get_text().find("(")!=-1):
            title_info = re.split('[](]',title.get_text())
